...
{
    [CustomEditor(typeof(VariableNode))]
    public class VariableNodeEditor : BaseNodeEditor
    {
        private VariableNode TargetNode { get { return (VariableNode) target; } }

        private List<NamedConnection> m_outputValues;

        private List<NamedConnection> OutputValues
        {
            get
            {
                if (m_outputValues != null) return m_outputValues;
                m_outputValues = new List<NamedConnection>();
                foreach (Socket socket in TargetNode.OutputSockets)
                    OutputValues.Add(NamedConnection.GetValue(socket));
                return m_outputValues;
            }
        }

        private void UpdateSocketValue(Socket socket, NamedConnection value)
        {
            NamedConnection.SetValue(socket, value);
        ...
        public override void OnInspectorGUI()
        {
            ...
            DrawHeader(Styles.GetStyle(Styles.StyleName.ComponentHeaderRandomNode), MenuUtils.VariableNode_Manual, MenuUtils.VariableNode_YouTube);
            DrawDebugMode(true);
        ...
        private void DrawOutputSockets()
        {
            ...
            List<NamedConnection> values = OutputValues;
                ...
                GUILayout.BeginHorizontal(GUILayout.Height(kNodeLineHeight));
                {
                    ...
                    GUILayout.BeginVertical();
                    {
                        GUILayout.BeginHorizontal(GUILayout.Height(kNodeLineHeight), GUILayout.ExpandWidth(true));
                        {
                            ...
                            GUILayout.Space(DGUI.Properties.Space(2));
                            DGUI.Label.Draw(UILabels.Name, Size.M, kNodeLineHeight);
                            GUILayout.Space(DGUI.Properties.Space());
                            ...
                            GUILayout.BeginVertical(GUILayout.Height(kNodeLineHeight));
                            {
                                ...
                                name = EditorGUILayout.TextField(GUIContent.none, name, GUILayout.Height(DGUI.Properties.SingleLineHeight)); //Draw output connection weight
                            }
